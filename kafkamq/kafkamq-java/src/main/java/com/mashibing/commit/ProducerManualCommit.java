 ...
 * version: 1.0
 */
public class ProducerManualCommit extends BaseTest {

    private static final Logger logger = LoggerFactory.getLogger(ProducerManualCommit.class);

    public static final String USER_TOPIC = "user003";
        ...
        try (KafkaProducer<String, User> producer = new KafkaProducer<>(props);) {
            ...
            for (int i = 0; i < 8; i++) {
                ProducerRecord<String, User> record = new ProducerRecord<>(USER_TOPIC
                        , User.builder().id(i + 1).name("commit" + (i + 1)).build());
...
//            sendBySync(producer, record); // 同步
     ...
     */
    private void sendBySync(Producer<String, User> producer, ProducerRecord<String, User> record) throws ExecutionException, InterruptedException {
        // 发送消息（同步）
        ...
        logger.info("topic: {}, key: {}, value: {}, partition: {}, offset: {}", record.topic(), record.key(), record.value(), partition, offset);
    }
     ...
     * create by: zhaosong 2024/11/12 14:09
     */
    private void sendByAsync(Producer<String, User> producer, ProducerRecord<String, User> record) throws ExecutionException, InterruptedException {
        // 发送消息（异步）
