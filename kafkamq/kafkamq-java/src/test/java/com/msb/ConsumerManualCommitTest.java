...
import com.mashibing.commit.ProducerManualCommit;
 ...
 * version: 1.0
 */
public class ConsumerManualCommitTest extends BaseTest {

    private static final Logger logger = LoggerFactory.getLogger(ConsumerManualCommitTest.class);
    ...
    public void consume() {
        ...
        try {
            // 订阅消费TOPIC
            consumer.assign(Collections.singleton(new TopicPartition(ProducerManualCommit.USER_TOPIC, 0)));
            // 循环拉取信息（监听）
            while (true) {
                // 每隔2秒，拉取准备就绪的信息列表
                ...
                if (null == recordList || recordList.isEmpty()) {
                    ...
                    try {
                        consumer.listTopics();
                        logger.info("[{}]kafka连通性正常",count);
                        TimeUnit.SECONDS.sleep(5);
                    } catch (Exception ex) {
                        logger.error("[{}]检查kafka连通性", count, ex);
                    }
                    continue;
                }
